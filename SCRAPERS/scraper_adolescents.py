#!/usr/bin/env python3
"""
SCRAPER AMAZON AMÉLIORÉ V3.0 - ADOLESCENTS
===================================================
Catégorie: Adolescents
URL: https://www.amazon.fr/s?k=Adolescents&i=stripbooks
...
class ScraperAmazonAdolescents(ScraperAmazonAmeliore):
    """
    Scraper spécialisé pour: Adolescents
    ...
    def __init__(self, mode_forcee: bool = False):
        """
        Initialise le scraper v3.0 pour Adolescents

        Args:
            mode_forcee (bool): Force la mise à jour de TOUS les livres existants (défaut: False)
        """
        super().__init__(
            nom_categorie="Adolescents",
            url_categorie="https://www.amazon.fr/s?k=Adolescents&i=stripbooks",
...
def main(mode_forcee: bool = False):
    ...
    """
    scraper = ScraperAmazonAdolescents(mode_forcee=mode_forcee)

    print(f"🚀 Démarrage du scraper v3.0 pour: Adolescents")
    ...
    if total_nouveaux > 0:
        print(f"\n✅ SUCCÈS: {total_nouveaux} nouveaux livres ajoutés pour Adolescents")
    else:
        print(f"\n✅ TERMINÉ: Base de données à jour pour Adolescents")
