#!/usr/bin/env python3
"""
SCRAPER AMAZON AMÉLIORÉ V3.0 - ALLEMAND
================================================
Catégorie: Allemand
URL: https://www.amazon.fr/s?k=Allemand&i=stripbooks
...
class ScraperAmazonAllemand(ScraperAmazonAmeliore):
    """
    Scraper spécialisé pour: Allemand
    ...
    def __init__(self, mode_forcee: bool = False):
        """
        Initialise le scraper v3.0 pour Allemand

        Args:
            mode_forcee (bool): Force la mise à jour de TOUS les livres existants (défaut: False)
        """
        super().__init__(
            nom_categorie="Allemand",
            url_categorie="https://www.amazon.fr/s?k=Allemand&i=stripbooks",
...
def main(mode_forcee: bool = False):
    ...
    """
    scraper = ScraperAmazonAllemand(mode_forcee=mode_forcee)

    print(f"🚀 Démarrage du scraper v3.0 pour: Allemand")
    ...
    if total_nouveaux > 0:
        print(f"\n✅ SUCCÈS: {total_nouveaux} nouveaux livres ajoutés pour Allemand")
    else:
        print(f"\n✅ TERMINÉ: Base de données à jour pour Allemand")
