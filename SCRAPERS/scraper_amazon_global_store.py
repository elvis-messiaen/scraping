#!/usr/bin/env python3
"""
SCRAPER AMAZON AMÉLIORÉ V3.0 - AMAZON GLOBAL STORE
===========================================================
Catégorie: Amazon Global Store
URL: https://www.amazon.fr/s?k=Amazon+Global+Store&i=stripbooks
...
class ScraperAmazonAmazonGlobalStore(ScraperAmazonAmeliore):
    """
    Scraper spécialisé pour: Amazon Global Store
    ...
    def __init__(self, mode_forcee: bool = False):
        """
        Initialise le scraper v3.0 pour Amazon Global Store

        Args:
            mode_forcee (bool): Force la mise à jour de TOUS les livres existants (défaut: False)
        """
        super().__init__(
            nom_categorie="Amazon Global Store",
            url_categorie="https://www.amazon.fr/s?k=Amazon+Global+Store&i=stripbooks",
...
def main(mode_forcee: bool = False):
    ...
    """
    scraper = ScraperAmazonAmazonGlobalStore(mode_forcee=mode_forcee)

    print(f"🚀 Démarrage du scraper v3.0 pour: Amazon Global Store")
    ...
    if total_nouveaux > 0:
        print(f"\n✅ SUCCÈS: {total_nouveaux} nouveaux livres ajoutés pour Amazon Global Store")
    else:
        print(f"\n✅ TERMINÉ: Base de données à jour pour Amazon Global Store")
