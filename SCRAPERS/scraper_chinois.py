#!/usr/bin/env python3
"""
SCRAPER AMAZON AMÉLIORÉ V3.0 - CHINOIS
===============================================
Catégorie: Chinois
URL: https://www.amazon.fr/s?k=Chinois&i=stripbooks
...
class ScraperAmazonChinois(ScraperAmazonAmeliore):
    """
    Scraper spécialisé pour: Chinois
    ...
    def __init__(self, mode_forcee: bool = False):
        """
        Initialise le scraper v3.0 pour Chinois

        Args:
            mode_forcee (bool): Force la mise à jour de TOUS les livres existants (défaut: False)
        """
        super().__init__(
            nom_categorie="Chinois",
            url_categorie="https://www.amazon.fr/s?k=Chinois&i=stripbooks",
...
def main(mode_forcee: bool = False):
    ...
    """
    scraper = ScraperAmazonChinois(mode_forcee=mode_forcee)

    print(f"🚀 Démarrage du scraper v3.0 pour: Chinois")
    ...
    if total_nouveaux > 0:
        print(f"\n✅ SUCCÈS: {total_nouveaux} nouveaux livres ajoutés pour Chinois")
    else:
        print(f"\n✅ TERMINÉ: Base de données à jour pour Chinois")
