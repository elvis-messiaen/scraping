#!/usr/bin/env python3
"""
SCRAPER AMAZON AMÉLIORÉ V3.0 - DEPUIS 3 MOIS
=====================================================
Catégorie: Depuis 3 mois
URL: https://www.amazon.fr/s?k=Depuis+3+mois&i=stripbooks
...
class ScraperAmazonDepuis3mois(ScraperAmazonAmeliore):
    """
    Scraper spécialisé pour: Depuis 3 mois
    ...
    def __init__(self, mode_forcee: bool = False):
        """
        Initialise le scraper v3.0 pour Depuis 3 mois

        Args:
            mode_forcee (bool): Force la mise à jour de TOUS les livres existants (défaut: False)
        """
        super().__init__(
            nom_categorie="Depuis 3 mois",
            url_categorie="https://www.amazon.fr/s?k=Depuis+3+mois&i=stripbooks",
...
def main(mode_forcee: bool = False):
    ...
    """
    scraper = ScraperAmazonDepuis3mois(mode_forcee=mode_forcee)

    print(f"🚀 Démarrage du scraper v3.0 pour: Depuis 3 mois")
    ...
    if total_nouveaux > 0:
        print(f"\n✅ SUCCÈS: {total_nouveaux} nouveaux livres ajoutés pour Depuis 3 mois")
    else:
        print(f"\n✅ TERMINÉ: Base de données à jour pour Depuis 3 mois")
