#!/usr/bin/env python3
"""
SCRAPER AMAZON AMÉLIORÉ V3.0 - ESPAGNOL
================================================
Catégorie: Espagnol
URL: https://www.amazon.fr/s?k=Espagnol&i=stripbooks
...
class ScraperAmazonEspagnol(ScraperAmazonAmeliore):
    """
    Scraper spécialisé pour: Espagnol
    ...
    def __init__(self, mode_forcee: bool = False):
        """
        Initialise le scraper v3.0 pour Espagnol

        Args:
            mode_forcee (bool): Force la mise à jour de TOUS les livres existants (défaut: False)
        """
        super().__init__(
            nom_categorie="Espagnol",
            url_categorie="https://www.amazon.fr/s?k=Espagnol&i=stripbooks",
...
def main(mode_forcee: bool = False):
    ...
    """
    scraper = ScraperAmazonEspagnol(mode_forcee=mode_forcee)

    print(f"🚀 Démarrage du scraper v3.0 pour: Espagnol")
    ...
    if total_nouveaux > 0:
        print(f"\n✅ SUCCÈS: {total_nouveaux} nouveaux livres ajoutés pour Espagnol")
    else:
