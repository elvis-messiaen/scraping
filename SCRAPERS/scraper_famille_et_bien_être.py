#!/usr/bin/env python3
"""
SCRAPER AMAZON AMÉLIORÉ V3.0 - FAMILLE ET BIEN-ÊTRE
============================================================
Catégorie: Famille et bien-être
URL: https://www.amazon.fr/s?k=Famille+et+bien-être&i=stripbooks
...
class ScraperAmazonFamilleetbienêtre(ScraperAmazonAmeliore):
    """
    Scraper spécialisé pour: Famille et bien-être
    ...
    def __init__(self, mode_forcee: bool = False):
        """
        Initialise le scraper v3.0 pour Famille et bien-être

        Args:
            mode_forcee (bool): Force la mise à jour de TOUS les livres existants (défaut: False)
        """
        super().__init__(
            nom_categorie="Famille et bien-être",
            url_categorie="https://www.amazon.fr/s?k=Famille+et+bien-être&i=stripbooks",
...
def main(mode_forcee: bool = False):
    ...
    """
    scraper = ScraperAmazonFamilleetbienêtre(mode_forcee=mode_forcee)

    print(f"🚀 Démarrage du scraper v3.0 pour: Famille et bien-être")
    ...
    if total_nouveaux > 0:
        print(f"\n✅ SUCCÈS: {total_nouveaux} nouveaux livres ajoutés pour Famille et bien-être")
    else:
        print(f"\n✅ TERMINÉ: Base de données à jour pour Famille et bien-être")
