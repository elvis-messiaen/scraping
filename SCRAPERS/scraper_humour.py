#!/usr/bin/env python3
"""
SCRAPER AMAZON AMÉLIORÉ V3.0 - HUMOUR
==============================================
Catégorie: Humour
URL: https://www.amazon.fr/s?k=Humour&i=stripbooks
...
class ScraperAmazonHumour(ScraperAmazonAmeliore):
    """
    Scraper spécialisé pour: Humour
    ...
    def __init__(self, mode_forcee: bool = False):
        """
        Initialise le scraper v3.0 pour Humour

        Args:
            mode_forcee (bool): Force la mise à jour de TOUS les livres existants (défaut: False)
        """
        super().__init__(
            nom_categorie="Humour",
            url_categorie="https://www.amazon.fr/s?k=Humour&i=stripbooks",
...
def main(mode_forcee: bool = False):
    ...
    """
    scraper = ScraperAmazonHumour(mode_forcee=mode_forcee)

    print(f"🚀 Démarrage du scraper v3.0 pour: Humour")
    ...
    if total_nouveaux > 0:
        print(f"\n✅ SUCCÈS: {total_nouveaux} nouveaux livres ajoutés pour Humour")
    else:
        print(f"\n✅ TERMINÉ: Base de données à jour pour Humour")
