#!/usr/bin/env python3
"""
SCRAPER AMAZON AMÉLIORÉ V3.0 - HÉBREU
==============================================
Catégorie: Hébreu
URL: https://www.amazon.fr/s?k=Hébreu&i=stripbooks
...
class ScraperAmazonHébreu(ScraperAmazonAmeliore):
    """
    Scraper spécialisé pour: Hébreu
    ...
    def __init__(self, mode_forcee: bool = False):
        """
        Initialise le scraper v3.0 pour Hébreu

        Args:
            mode_forcee (bool): Force la mise à jour de TOUS les livres existants (défaut: False)
        """
        super().__init__(
            nom_categorie="Hébreu",
            url_categorie="https://www.amazon.fr/s?k=Hébreu&i=stripbooks",
...
def main(mode_forcee: bool = False):
    ...
    """
    scraper = ScraperAmazonHébreu(mode_forcee=mode_forcee)

    print(f"🚀 Démarrage du scraper v3.0 pour: Hébreu")
    ...
    if total_nouveaux > 0:
        print(f"\n✅ SUCCÈS: {total_nouveaux} nouveaux livres ajoutés pour Hébreu")
    else:
        print(f"\n✅ TERMINÉ: Base de données à jour pour Hébreu")
