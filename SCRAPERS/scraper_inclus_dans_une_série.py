#!/usr/bin/env python3
"""
SCRAPER AMAZON AMÉLIORÉ V3.0 - INCLUS DANS UNE SÉRIE
=============================================================
Catégorie: Inclus dans une série
URL: https://www.amazon.fr/s?k=Inclus+dans+une+série&i=stripbooks
...
class ScraperAmazonInclusdansunesérie(ScraperAmazonAmeliore):
    """
    Scraper spécialisé pour: Inclus dans une série
    ...
    def __init__(self, mode_forcee: bool = False):
        """
        Initialise le scraper v3.0 pour Inclus dans une série

        Args:
            mode_forcee (bool): Force la mise à jour de TOUS les livres existants (défaut: False)
        """
        super().__init__(
            nom_categorie="Inclus dans une série",
            url_categorie="https://www.amazon.fr/s?k=Inclus+dans+une+série&i=stripbooks",
...
def main(mode_forcee: bool = False):
    ...
    """
    scraper = ScraperAmazonInclusdansunesérie(mode_forcee=mode_forcee)

    print(f"🚀 Démarrage du scraper v3.0 pour: Inclus dans une série")
    ...
    if total_nouveaux > 0:
        print(f"\n✅ SUCCÈS: {total_nouveaux} nouveaux livres ajoutés pour Inclus dans une série")
    else:
        print(f"\n✅ TERMINÉ: Base de données à jour pour Inclus dans une série")
