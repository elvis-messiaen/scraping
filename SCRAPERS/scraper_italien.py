#!/usr/bin/env python3
"""
SCRAPER AMAZON AMÉLIORÉ V3.0 - ITALIEN
===============================================
Catégorie: Italien
URL: https://www.amazon.fr/s?k=Italien&i=stripbooks
...
class ScraperAmazonItalien(ScraperAmazonAmeliore):
    """
    Scraper spécialisé pour: Italien
    ...
    def __init__(self, mode_forcee: bool = False):
        """
        Initialise le scraper v3.0 pour Italien

        Args:
            mode_forcee (bool): Force la mise à jour de TOUS les livres existants (défaut: False)
        """
        super().__init__(
            nom_categorie="Italien",
            url_categorie="https://www.amazon.fr/s?k=Italien&i=stripbooks",
...
def main(mode_forcee: bool = False):
    ...
    """
    scraper = ScraperAmazonItalien(mode_forcee=mode_forcee)

    print(f"🚀 Démarrage du scraper v3.0 pour: Italien")
    ...
    if total_nouveaux > 0:
        print(f"\n✅ SUCCÈS: {total_nouveaux} nouveaux livres ajoutés pour Italien")
    else:
        print(f"\n✅ TERMINÉ: Base de données à jour pour Italien")
