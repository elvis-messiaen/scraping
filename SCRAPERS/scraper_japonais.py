#!/usr/bin/env python3
"""
SCRAPER AMAZON AMÉLIORÉ V3.0 - JAPONAIS
================================================
Catégorie: Japonais
URL: https://www.amazon.fr/s?k=Japonais&i=stripbooks
...
class ScraperAmazonJaponais(ScraperAmazonAmeliore):
    """
    Scraper spécialisé pour: Japonais
    ...
    def __init__(self, mode_forcee: bool = False):
        """
        Initialise le scraper v3.0 pour Japonais

        Args:
            mode_forcee (bool): Force la mise à jour de TOUS les livres existants (défaut: False)
        """
        super().__init__(
            nom_categorie="Japonais",
            url_categorie="https://www.amazon.fr/s?k=Japonais&i=stripbooks",
...
def main(mode_forcee: bool = False):
    ...
    """
    scraper = ScraperAmazonJaponais(mode_forcee=mode_forcee)

    print(f"🚀 Démarrage du scraper v3.0 pour: Japonais")
    ...
    if total_nouveaux > 0:
        print(f"\n✅ SUCCÈS: {total_nouveaux} nouveaux livres ajoutés pour Japonais")
    else:
        print(f"\n✅ TERMINÉ: Base de données à jour pour Japonais")
