#!/usr/bin/env python3
"""
SCRAPER AMAZON AMÉLIORÉ V3.0 - LIVRE AUDIO
===================================================
Catégorie: Livre audio
URL: https://www.amazon.fr/s?k=Livre+audio&i=stripbooks
...
class ScraperAmazonLivreaudio(ScraperAmazonAmeliore):
    """
    Scraper spécialisé pour: Livre audio
    ...
    def __init__(self, mode_forcee: bool = False):
        """
        Initialise le scraper v3.0 pour Livre audio

        Args:
            mode_forcee (bool): Force la mise à jour de TOUS les livres existants (défaut: False)
        """
        super().__init__(
            nom_categorie="Livre audio",
            url_categorie="https://www.amazon.fr/s?k=Livre+audio&i=stripbooks",
...
def main(mode_forcee: bool = False):
    ...
    """
    scraper = ScraperAmazonLivreaudio(mode_forcee=mode_forcee)

    print(f"🚀 Démarrage du scraper v3.0 pour: Livre audio")
    ...
    if total_nouveaux > 0:
        print(f"\n✅ SUCCÈS: {total_nouveaux} nouveaux livres ajoutés pour Livre audio")
    else:
        print(f"\n✅ TERMINÉ: Base de données à jour pour Livre audio")
