#!/usr/bin/env python3
"""
SCRAPER AMAZON AMÉLIORÉ V3.0 - NATURE ET ANIMAUX
=========================================================
Catégorie: Nature et animaux
URL: https://www.amazon.fr/s?k=Nature+et+animaux&i=stripbooks
...
class ScraperAmazonNatureetanimaux(ScraperAmazonAmeliore):
    """
    Scraper spécialisé pour: Nature et animaux
    ...
    def __init__(self, mode_forcee: bool = False):
        """
        Initialise le scraper v3.0 pour Nature et animaux

        Args:
            mode_forcee (bool): Force la mise à jour de TOUS les livres existants (défaut: False)
        """
        super().__init__(
            nom_categorie="Nature et animaux",
            url_categorie="https://www.amazon.fr/s?k=Nature+et+animaux&i=stripbooks",
...
def main(mode_forcee: bool = False):
    ...
    """
    scraper = ScraperAmazonNatureetanimaux(mode_forcee=mode_forcee)

    print(f"🚀 Démarrage du scraper v3.0 pour: Nature et animaux")
    ...
    if total_nouveaux > 0:
        print(f"\n✅ SUCCÈS: {total_nouveaux} nouveaux livres ajoutés pour Nature et animaux")
    else:
        print(f"\n✅ TERMINÉ: Base de données à jour pour Nature et animaux")
