#!/usr/bin/env python3
"""
SCRAPER AMAZON AMÉLIORÉ V3.0 - PORTUGAIS
=================================================
Catégorie: Portugais
URL: https://www.amazon.fr/s?k=Portugais&i=stripbooks
...
class ScraperAmazonPortugais(ScraperAmazonAmeliore):
    """
    Scraper spécialisé pour: Portugais
    ...
    def __init__(self, mode_forcee: bool = False):
        """
        Initialise le scraper v3.0 pour Portugais

        Args:
            mode_forcee (bool): Force la mise à jour de TOUS les livres existants (défaut: False)
        """
        super().__init__(
            nom_categorie="Portugais",
            url_categorie="https://www.amazon.fr/s?k=Portugais&i=stripbooks",
...
def main(mode_forcee: bool = False):
    ...
    """
    scraper = ScraperAmazonPortugais(mode_forcee=mode_forcee)

    print(f"🚀 Démarrage du scraper v3.0 pour: Portugais")
    ...
    if total_nouveaux > 0:
        print(f"\n✅ SUCCÈS: {total_nouveaux} nouveaux livres ajoutés pour Portugais")
    else:
        print(f"\n✅ TERMINÉ: Base de données à jour pour Portugais")
