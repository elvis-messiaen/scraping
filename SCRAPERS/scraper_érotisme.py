#!/usr/bin/env python3
"""
SCRAPER AMAZON AMÉLIORÉ V3.0 - ÉROTISME
================================================
Catégorie: Érotisme
URL: https://www.amazon.fr/s?k=Érotisme&i=stripbooks
...
class ScraperAmazonÉrotisme(ScraperAmazonAmeliore):
    """
    Scraper spécialisé pour: Érotisme
    ...
    def __init__(self, mode_forcee: bool = False):
        """
        Initialise le scraper v3.0 pour Érotisme

        Args:
            mode_forcee (bool): Force la mise à jour de TOUS les livres existants (défaut: False)
        """
        super().__init__(
            nom_categorie="Érotisme",
            url_categorie="https://www.amazon.fr/s?k=Érotisme&i=stripbooks",
...
def main(mode_forcee: bool = False):
    ...
    """
    scraper = ScraperAmazonÉrotisme(mode_forcee=mode_forcee)

    print(f"🚀 Démarrage du scraper v3.0 pour: Érotisme")
    ...
    if total_nouveaux > 0:
        print(f"\n✅ SUCCÈS: {total_nouveaux} nouveaux livres ajoutés pour Érotisme")
    else:
        print(f"\n✅ TERMINÉ: Base de données à jour pour Érotisme")
